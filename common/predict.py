...
def get_loss_masked_batch(inputs, logits, return_not_sum=False):
    ...
    if return_not_sum:
        return avg_loss_per_sample.to("cpu"), perplexity_per_sample.to("cpu"), per_token_loss.to("cpu"), torch.exp(
...
def explain_masked(
    ...
    for i in range(seq_len):
        ...
        if current_token_id == tokenizer.cls_token_id or \
                current_token_id == tokenizer.sep_token_id:
    ...
    for batch in tqdm(dataloader, total=len(dataloader)):
        ...
        with torch.no_grad():
            ...
            indices = mask.float().argmax(dim=1, keepdim=True).to(per_sample_loss.device)
...
def get_batch_predictions_masked_strided(
    ...
    """
    model = AutoModelForMaskedLM.from_pretrained(model_path, **model_load_settings).to("cuda")
    ...
    tokenized_dataset.set_format(type='torch', columns=['input_ids', 'attention_mask', 'labels', 'original_index'])
                ...
                if log_times:
                    times.append(
                        {"original_index": indices[i].item(),
                         ...
                         "tokens_count": (inputs["attention_mask"][i] == 1).sum().item()})
...
def get_loss_perplexity_causal_batch(
    ...
    """
    loss_fct = nn.CrossEntropyLoss(reduction='none', ignore_index=-100)  # Define loss function
    ...
    if return_not_sum:
        return per_sample_loss, per_sample_perplexity, per_sample_loss_all, torch.exp(per_sample_loss_all)
...
def get_batch_predictions_causal(
    ...
    model = AutoModelForCausalLM.from_pretrained(model_path, **model_load_settings).to("cuda")
    tokenizer = AutoTokenizer.from_pretrained(model_path)
    df = pandas.read_csv(dataset_path)
    ...
    for row_index in tqdm(range(0, len(df), batch_size), desc="Running Predictions"):
        ...
        per_sample_loss, per_sample_perplexity, all_loss, all_perplexity \
            = get_loss_perplexity_causal_batch(inputs, logits, pad_token_id, return_not_sum=True)
            ...
            if log_times:
                times.append(
                    {"original_index": indices[index],
                     ...
                     "tokens_count": (inputs["attention_mask"][index] == 1).sum().item()})
...
def explain_causal(
    ...
    avg_loss, avg_perplexity, all_loss, all_perplexity = get_loss_perplexity_causal_batch(
        inputs,
        logits,
        pad_token_id,
        return_not_sum=True
...
def estimate_threshold_from_predictions(
    ...
    if method == "gaussian":
        ...
        if plot:
            ...
            plt.plot(x, p, 'k', linewidth=2, label=rf'Fit results: $\mu$={mu:.4f}, $\sigma$={std:.4f}')
        ...
        if plot:
            ...
            percentile_values = {p: np.percentile(df[eval_metric].tolist(), p) for p in percentiles}
            ...
            for p in percentiles:
                ...
                plt.axvline(x=val, color=colors[p], linestyle=styles[p], linewidth=2,
