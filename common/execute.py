...
def execute(
    ...
    prediction_dataset_name = os.path.splitext(
        os.path.basename(prediction_dataset_path)
    )[0]
    ...
    prediction_output_path = os.path.join(
        prediction_dir_path, f"pred_{prediction_dataset_name}{predict_name_suffix}.csv"
    )
    ...
    validate_prediction_output_path = os.path.join(
        prediction_dir_path, f"pred_{validate_dataset_name}{predict_name_suffix}.csv"
    )
    ...
    if do_evaluate:
        ...
        if evaluate_params is None:
            ...
            for i in range(1, 100, 1):
                params.append(
                    (3000, 0.01 * i, "loss", True, False, 0.0, True, True, False)
                )
        else:
        ...
        if eval_auto_threshold_est:
            estimated_threshold = estimate_threshold_from_predictions(
                validate_prediction_output_path,
                method=auto_threshold_est_method,
                k=0.5,
                plot=True,
            )

            params.append(
                (3000, estimated_threshold, "loss", True, False, 0.0, True, True, True)
            )
        ...
        eval_output_path = os.path.join(
            evaluation_dir_path, f"eval_{prediction_dataset_name}{eval_name_suffix}.csv"
        )

        evaluate(prediction_output_path, truth_dataset_path, params, eval_output_path)

    try:
        auc_roc, auc_pr, auc_roc_ppl, auc_pr_ppl = get_AUC_ROC_from_predictions(
            prediction_output_path, truth_dataset_path
        )
